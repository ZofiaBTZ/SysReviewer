...
def txt2textRank(txt_dir, textRank_out):
    # retrieve each of the articles
    articles = os.listdir(txt_dir)
    for article in articles:
        print('Reading articles/' + article)
        article_file = io.open(txt_dir + "/" + article, 'r')
        text = article_file.read()
        keyphrases = extract_key_phrases(text)
        summary = extract_sentences(text)
        write_files(summary, keyphrases, article)

# combine summaries and keywords from all files
